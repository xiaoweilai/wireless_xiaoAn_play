...
import com.all.wirelessplayer.SDLActivity.MyHandler;
		...
		public void run() {
	         try {
				...
				if(!mReceiveSuccessedFlag){
					PublicFunction.writeFile(mContext, getLogFileName(), "�߳������쳣   run " + Thread.currentThread().getName());
					System.out.println("�߳������쳣   run " + Thread.currentThread().getName());
					mHandler.sendEmptyMessage(6);
					if(receiveImag != null){
						receiveImag.interrupt();
					}
...
//						PublicFunction.writeFile(mContext, System.currentTimeMillis()+"file.txt", mGetByteMessage);
			...
			PublicFunction.writeFile(mContext, getLogFileName(), "socket input output�쳣" + Thread.currentThread().getName());
			PublicFunction.writeFile(mContext, getLogFileName(), "�߳������쳣" + Thread.currentThread().getName());
			System.out.println("�߳������쳣  receiveMessage" + Thread.currentThread().getName());
			try {
				if (null != input)
					input.close();
				if (null != output)
